...
$slides_number = count($gallery);
//var_dump($gallery);
?>
<div class="image-slider slider <?php echo !empty($block['className']) ? $block['className'] : '' ?>" data-slide="1">
  <div class="slider__items">
    <?php foreach($gallery as $image): ?> 
    <div class="image-slider__item slider__item" style="background-image: url('<?php echo $image['url']; ?>'); <?php echo $image['background_size'] ? 'background-size:' . $image['background_size'] . ';' : '' ?>">
  ...
  <nav class="image-slider__nav slider__nav container">
    ...
    <button class="image-slider__nav-item slider__nav-item" data-target-slide="<?php echo $i ?>">
